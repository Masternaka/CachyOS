...
usage() {
  cat <<EOF
Usage: $(basename "$0") [options] [paquet-kde...]

Installe des paquets KDE/Plasma depuis les depots officiels avec pacman.
Sans paquet en argument, le script charge liste des paquets/kde-packages.conf.
...
load_config() {
  local file="${CONFIG_FILE:-$SCRIPT_DIR/liste des paquets/kde-packages.conf}"
...
main() {
  ...
  [[ ${#packages[@]} -gt 0 ]] || fatal "Aucun paquet KDE a installer"
  ...
  log_info "Installation des paquets KDE/Plasma"
