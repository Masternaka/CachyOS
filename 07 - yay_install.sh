...
# Installer yay si manquant (depuis les dépôts CachyOS)
install_yay() {
  if ! command -v yay &>/dev/null; then
    echo -e "${YELLOW}yay non détecté. Installation depuis les dépôts CachyOS...${RESET}"
    if [ "$DRY_RUN" = false ]; then
      pacman -S --noconfirm --needed yay-bin
    else
      echo "DRY-RUN: pacman -S --noconfirm --needed yay-bin"
    fi
  else
...
show_help() {
  ...
  echo ""
  echo "UTILISATION:"
  echo "  sudo ./07 - yay_install.sh [options]"
  echo ""
  echo "OPTIONS:"
  ...
  echo "  - Navigateur: "
  echo "  - Développement: gitkraken, fresh-editor"
  echo "  - Multimédia: deadbeef, spotify"
  echo "  - Office et notes: helixnotes, wps-office"
  echo "  - AI: deepseek-reasonix-tui, deepseek-reasonix-desktop"
}
